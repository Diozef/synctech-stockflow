-- Remove duplicate triggers/functions on sale_items
DO $$
BEGIN
...
-- Add installment_id column to financial_transactions
DO $$
BEGIN
...
-- Create function to generate financial_transaction when installment is marked as paid
...
BEGIN
  ...
  -- Detect transition to paid (accepts 'pago' or 'paid')
  IF (v_status_new = 'pago' OR v_status_new = 'paid') AND (v_status_old IS NULL OR (v_status_old != 'pago' AND v_status_old != 'paid')) THEN
    ...
    -- Protection against duplication: insert only if there's no transaction linked to this installment
...
-- Create trigger only if installments table exists and trigger doesn't exist
DO $$
BEGIN
